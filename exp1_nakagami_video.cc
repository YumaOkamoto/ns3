...
public:
  ...
  void Setup (uint32_t totalSegments,uint32_t activeChannels) {
...
public:
  ...
  void Setup (Ipv4Address dest, uint32_t packetSize, uint32_t numChannels, Time interval, int mode, uint32_t calculatedN) {
    ...
    if (mode == 0) { // FB法
      ...
      m_totalSegments = N_val;
    // チャネル数 k が セグメント数 N と同じになるようにループ            
...
int main (int argc, char *argv[]) {
  ...
  uint32_t serverChannels;
  if (mode == 0) {
    serverChannels = kChannels; // FB法は本来の k 本で送る
  } else if (mode == 2) {
    serverChannels = totalSegs; // 等分割法は N 本で並列に送る
  } else {
    serverChannels = 1;
  }
  Ptr<ServerApp> serverApp = CreateObject<ServerApp> ();
  serverApp->Setup (Ipv4Address ("224.1.1.1"), pSize, serverChannels, Seconds(interval), mode, totalSegs);
  ...
  for (uint32_t i = 0; i < nNodes; ++i) {
    ...
    if (mode == 0) {
      monitorChannels = kChannels; // FB法は k 本
    } else if (mode == 2) {
      monitorChannels = totalSegs; // 等分割法(N=k設定)は N 本
    } else {
      monitorChannels = 1;         // 集中型は 1 本
    }
    clientApp->Setup (totalSegs,monitorChannels);
