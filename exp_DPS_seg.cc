...
public:
  uint32_t GetRawReceivedCount () const { return m_rawReceivedCount; }

  ClientApp () :m_rawReceivedCount(0), m_receivedCount (0), m_allReceived (false) {}
...
public:
  uint32_t GetTotalSent() const { return m_sentCount; }
...
int main (int argc, char *argv[]) {
  ...
  std::string modeName = (mode == 0) ? "FB法" : (mode == 1 ? "放送配信" : "等分割法");
  ...
  for (uint32_t i = 0; i < clientNodes.GetN(); ++i) {
      ...
      double ArvRate = ((double)received / totalSegs) * 100;
      totalArvRateSum += ArvRate;
  }
  ...
  // 平均欠損率の計算
  double avgArv = totalArvRateSum / nNodes;
 
  //パケット欠損率の計算
  uint32_t totalSentByServer = serverApp->GetTotalSent();
  uint32_t totalReceivedByAllClients = 0;

for (uint32_t i = 0; i < clientNodes.GetN(); ++i) {
    Ptr<ClientApp> app = DynamicCast<ClientApp>(clientNodes.Get(i)->GetApplication(0));
    totalReceivedByAllClients += app->GetRawReceivedCount();
}
...
NS_LOG_UNCOND("【パケット欠損率】: " << LossRate << "%");
  ...
  // Excel用最終出力
  NS_LOG_UNCOND ("\n" << "=== 最終結果サマリー (Excel用) ===");
  NS_LOG_UNCOND ("手法,ノード数,チャネル数(k),総セグメント数(N),スロット間隔,帯域幅,平均待ち時間,平均到達率,パケット欠損率,距離");
  NS_LOG_UNCOND (modeName << "," << nNodes << "," << actualk << "," << totalSegs << "," << interval << "," << dataRate << "," << avg << "," << avgArv << "%" << "," << LossRate << "%" << "," << dist );
